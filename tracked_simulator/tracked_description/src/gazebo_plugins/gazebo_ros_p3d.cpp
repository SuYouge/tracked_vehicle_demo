...
// void GazeboRosP3D::chatterCallback(const nav_msgs::Odometry::ConstPtr &_odom){
//     this->bias_.X() = _odom->pose.pose.position.x;
//     this->bias_.Y() = _odom->pose.pose.position.y;
//     this->bias_.Z() = _odom->pose.pose.position.z;
// }
...
void GazeboRosP3D::Load(physics::ModelPtr _parent, sdf::ElementPtr _sdf)
{
  ...
  this->update_connection_ = event::Events::ConnectWorldUpdateBegin(
    ...
    // ros::Subscriber sub = this->rosnode_->subscribe<nav_msgs::Odometry>(this->topic_name_, 1, boost::bind(&GazeboRosP3D::chatterCallback, this, _1));
}
  ...
  if (this->pub_.getNumSubscribers() > 0)
  {
    ...
    if (tmp_dt != 0)
    {
      ...
      if (this->topic_name_ != "")
      {
        ...
        pose.Pos() = pose.Pos() + this->offset_.Pos() - this->bias_;
        // apply rpy offsets
